...
def build_network_status_html(
    contract,
    users,
    display_name_fn: Optional[Callable[[str], str]] = None,
) -> str:
    """
    Build the HTML table describing the current network status.
    """
    display_name = display_name_fn or (lambda name: name)
    ...
    for addr, data in users.items():
        ...
        display_label = display_name(data["name"])
        user_stats.append(
            {
                "name": display_label,
                "rank": rank_display,
    ...
    for stat in user_stats:
        ...
        prev_rank_cell_style = (
            "color: #666; font-style: italic;" if stat["prev_rank"] != "-" else "color: #999;"
        )
        prev_score_cell_style = (
            "color: #666; font-style: italic;" if stat["prev_score"] != 0 else "color: #999;"
        )
...
def build_network_figure(
    ...
    users,
    display_name_fn: Optional[Callable[[str], str]] = None,
    focus_node_idx: Optional[int] = None,
):
    """
    Build the plotly Figure representing the current network graph.
    ...
    if focus_node_idx is not None and focus_node_idx in contract.network.nodes():
        ...
        if connected_nodes:
            graph = contract.network.subgraph(list(connected_nodes)).copy()
        else:
            graph = contract.network.subgraph([focus_node]).copy()
    else:
        graph = contract.network.copy()
    ...
    for idx in node_order:
        ...
        if address:
            ...
            display_label = display_name(user_name)
            ...
            labels[idx] = f"{display_label}<br>Rank: {rank_display}<br>Score: {score_val:,.0f}"
            node_scores[idx] = score_val
            node_rank_values[idx] = rank
            node_names[idx] = display_label
        else:
    ...
    for idx in node_order:
        ...
        color = node_colors_final[node_order.index(idx)]
        size = node_sizes_final[node_order.index(idx)]

        if idx == focus_node_idx:
            node_x_focus.append(x)
            node_y_focus.append(y)
            node_text_focus.append(label)
            node_colors_focus.append(color)
            node_sizes_focus.append(size)
        else:
            node_x_normal.append(x)
            node_y_normal.append(y)
            node_text_normal.append(label)
            node_colors_normal.append(color)
            node_sizes_normal.append(size)
    ...
    legend_annotations.append(
        dict(
            text="Node color represents rank (lighter = better rank), size represents score (bigger = higher score).",
            showarrow=False,
            xref="paper",
            yref="paper",
            x=0.005,
...
def show_network_graph(contract, users, ui):
    ...
    focus_node_idx = getattr(ui, "focus_node_idx", None)
    display_name_fn = getattr(ui, "_get_display_name", None)
    figure = build_network_figure(contract, users, display_name_fn, focus_node_idx)
    if figure is None:
